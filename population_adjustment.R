...
# also for MCMC posterior sampling in MIM data synthesis stage 
...
N_star <- 1000 # size of simulated BC pseudo-population (synthetic datasets for MIM); high for small MC error
# MCMC info for Bayesian G-computation and MIM
...
M <- 1000 # Number of syntheses used for analysis for MIM (set high to minimize Monte Carlo error across seeds)
...
mim.wrapper <- function(data.AC, data.BC, M, n.chains, warmup, iters,
                        N_star, allocation) {
  # Inputs: data.AC - AC individual patient-level data; data.BC - BC aggregate-level data;
  # M - number of syntheses; n.chains, warmup, iters - MCMC info
  ...
  x_star <- as.data.frame(rMvdc(N_star, mvd))
  colnames(x_star) <- c("X1", "X2", "X3", "X4")   
  # first-stage logistic regression model fitted to original data using MCMC
  outcome.model <- stan_glm(y~X3+X4+trt*X1+trt*X2,
                            data=data.AC, family=binomial, algorithm="sampling",
                            iter=iters, warmup=warmup, chains=n.chains, 
                            # thin to use M independent samples in analysis stage 
                            thin=(n.chains*(iters-warmup))/M) # run Stan model
  N_active <- round(N_star*allocation) # number of patients in synthesis under active treatment
  N_control <- N_star - N_active # number of patients in synthesis under control
  ...
  hats_v <- unlist(lapply(reg2.fits, function(fit) vcov(fit)["trt_star", "trt_star"]))
  ...
  b <- var(hats_delta_AC) # between variance (sample variance of point estimates)
  ## pooling + indirect comparison (combining rules)
  # average of point estimates is the relative effect for A vs. C
  hat.Delta.AC <- bar_delta_AC
  hat.var.Delta.AC <- (1+(1/M))*b-bar_v # variance combining rule for A vs. C
  negative.var <- as.numeric(hat.var.Delta.AC<0) # 1 if variance negative
  # ad hoc fix - set variance to zero if negative
  ...
  hat.Delta.AB <- hat.Delta.AC - hat.Delta.BC # A vs. B
  hat.var.Delta.AB <- hat.var.Delta.AC + hat.var.Delta.BC  
  list(hat.Delta.AB, hat.var.Delta.AB, negative.var)
}
...
for(i in 1:scenarios) {
  ...
  ### Multiple imputation marginalization (MIM)
  mim.results <- foreach(j=1:replicates, .combine='comb', .multicombine=TRUE,
                                 .init=list(list(),list(), list()), .options.snow=opts,
                                 .packages=c("copula", "rstanarm")) %dopar% {
                                   results <- mim.wrapper(IPD.AC[[j]], ALD.BC[[j]], M,
                                                          n.chains, warmup, iters, N_star,
                                                          allocation)
                                   return(results)
                                 }
